"""Download a run's artifact outputs (images now, videos / any File|Dir later)
out of the Flyte devbox blob store to your local disk.

Walks EVERY action in the run (not just the root), so it works for `compare`
runs (per-model images live in the child `generate_for_model` actions) as well as
`generate_one`. It skips weight-fetch tasks and only pulls media files, so the
multi-GB `.safetensors` Dirs never come down.
...
Usage:
    ...
    python download_outputs.py <run_name> --ext .mp4,.png  # custom extensions
